...
class ExperimentConfig:
    ...
    output_dir: str = "./outputs_opt_lmhead_stage1-v6"
    seed: int = 42
...
def scalar_quantize_tensor_with_scale(tensor: torch.Tensor, bits: int, scale: torch.Tensor) -> torch.Tensor:
    if bits < 1:
        raise ValueError(f"bits must be >=1, got {bits}")
    qmax = float(2 ** bits - 1)
    qmin = float(-(2 ** bits))
    safe_scale = torch.clamp(scale.to(dtype=tensor.dtype, device=tensor.device), min=1e-8)
    q = torch.round(tensor / safe_scale)
...
def scalar_quantize_weight_maxabs(weight: torch.Tensor, bits: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if float(max_abs.item()) == 0.0:
        ...
        scale = max_abs / qmax
    w_q = scalar_quantize_tensor_with_scale(weight, bits, scale)
    return w_q, torch.clamp(scale, min=1e-8)


class ScalarQuantizedWLMHead(nn.Module):
...
class ScalarQuantizedXWLMHead(nn.Module):
    def __init__(self, weight_q: torch.Tensor, x_scale: torch.Tensor, bits: int, bias: Optional[torch.Tensor] = None):
        super().__init__()
        self.register_buffer("weight_q", weight_q.detach().clone())
    ...
    def _quantize_x(self, hidden_states: torch.Tensor) -> torch.Tensor:
        qmax = float(2 ** self.bits - 1)
        qmin = float(-(2 ** self.bits))
        safe_scale = torch.clamp(self.x_scale.to(device=hidden_states.device, dtype=hidden_states.dtype), min=1e-8)
        q = torch.round(hidden_states / safe_scale)
        q = torch.clamp(q, qmin, qmax)
        return q * safe_scale

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        x_q = self._quantize_x(hidden_states)
...
def build_analysis_summary(artifacts: "ExperimentArtifacts") -> str:
    ...
    lines.append("")
    lines.append("重建与近似误差（SQ-W baseline）")
    for k, v in artifacts.sq_w_metrics.items():
        lines.append(f"- {k}: {v:.6f}")
    lines.append("")
    lines.append("重建与近似误差（SQ-XW baseline）")
...
def compute_sq_metrics(
    ...
    if x_scale is not None:
        if bits is None:
            raise ValueError("bits must be provided when x_scale is not None")
        X_q = scalar_quantize_tensor_with_scale(X, bits, x_scale.to(device=device, dtype=dtype))
    ...
    for start in range(0, X.shape[1], chunk_tokens):
        ...
        X_chunk = X[:, start:end]
        Xq_chunk = X_q[:, start:end]
        L_true = X_chunk.T @ W
        L_sq = Xq_chunk.T @ Wq_cols
        diff = L_true - L_sq
        numer += float(torch.sum(diff * diff).item())
        denom += float(torch.sum(L_true * L_true).item())
    ...
    if x_scale is not None:
        logger.info(
            "%s 误差计算完成 | rel_recon_error_x=%.6f rel_recon_error_w=%.6f rel_logits_error=%.6f elapsed=%.3fs",
            tag, err_x, err_w, err_ip, elapsed
        )
        return {
            "rel_recon_error_x": err_x,
            "rel_recon_error_w": err_w,
    ...
    logger.info(
        "%s 误差计算完成 | rel_recon_error_w=%.6f rel_logits_error=%.6f elapsed=%.3fs",
        tag, err_w, err_ip, elapsed
    )
    return {
        "rel_recon_error_w": err_w,
        "rel_logits_error": err_ip,
...
def build_sq_baselines_from_model(
    model,
    X_calib: torch.Tensor,
    config: ExperimentConfig,
    logger: logging.Logger,
) -> Tuple[nn.Module, nn.Module, Dict[str, float], Dict[str, float], Dict[str, object]]:
    logger.info("开始构建 SQ baselines（SQ-W 与 SQ-XW）。")
    ...
    weight_q, w_scale = scalar_quantize_weight_maxabs(weight.to(dtype=q_dtype, device=weight.device), bits)

    X_calib_dev = X_calib.to(device=weight.device, dtype=q_dtype)
    x_max_abs = torch.max(torch.abs(X_calib_dev))
    qmax = float(2 ** bits - 1)
    if float(x_max_abs.item()) == 0.0:
        x_scale = torch.tensor(1.0, dtype=q_dtype, device=weight.device)
    else:
        x_scale = torch.clamp(x_max_abs / qmax, min=1e-8)

    sq_w_head = ScalarQuantizedWLMHead(weight_q=weight_q, bias=bias)
    sq_w_head.to(next(model.parameters()).device)
    sq_xw_head = ScalarQuantizedXWLMHead(weight_q=weight_q, x_scale=x_scale, bits=bits, bias=bias)
    sq_xw_head.to(next(model.parameters()).device)

    W_cols = weight.detach().T.cpu()
    sq_w_metrics = compute_sq_metrics(X_calib, W_cols, weight_q, logger, tag="sq_w")
    sq_xw_metrics = compute_sq_metrics(X_calib, W_cols, weight_q, logger, x_scale=x_scale, bits=bits, tag="sq_xw")
    sq_info = {
        "bitwidth": bits,
        "sq_w_scale": float(w_scale.item()),
        "sq_x_scale": float(x_scale.item()),
        "sq_weight_q": tensor_stats(weight_q),
    }
    logger.info(
        "SQ baselines 构建完成 | bits=%d sq_w_scale=%.8f sq_x_scale=%.8f",
        bits, float(w_scale.item()), float(x_scale.item())
    )
    return sq_w_head, sq_xw_head, sq_w_metrics, sq_xw_metrics, sq_info
