...
class ExperimentConfig:
    ...
    output_dir: str = "./outputs_opt_last_block_qkvo_stage1-v1"
    seed: int = 42
...
def setup_logger(output_dir: Path) -> logging.Logger:
    logger = logging.getLogger(f"opt_last_block_qkvo_stage1_{output_dir.resolve()}")
...
def build_analysis_summary(artifacts: "ExperimentArtifacts") -> str:
    ...
    lines.append(f"- SQ-QKVO baseline PPL: {artifacts.sq_baseline_ppl:.6f}")
    lines.append(f"- Ours-QKVO quantized PPL: {artifacts.quantized_ppl:.6f}")
...
def get_last_block_attention_targets(model, target_linear_names: Tuple[str, ...], block_index: int) -> Tuple[nn.Module, Dict[str, nn.Module]]:
    ...
    for name in target_linear_names:
        if not hasattr(attn, name):
            raise AttributeError(f"Last block self_attn has no linear named: {name}")
        module = getattr(attn, name)
        if not isinstance(module, nn.Linear):
            raise TypeError(f"Target module {name} is not nn.Linear, got {type(module)}")
        modules[name] = module
    return attn, modules
...
def replace_target_modules(attn_module: nn.Module, replacements: Dict[str, nn.Module]) -> Dict[str, nn.Module]:
    original_modules: Dict[str, nn.Module] = {}
    for layer_name, replacement in replacements.items():
        original_modules[layer_name] = getattr(attn_module, layer_name)
        setattr(attn_module, layer_name, replacement)
    return original_modules



def restore_target_modules(attn_module: nn.Module, original_modules: Dict[str, nn.Module]) -> None:
    for layer_name, original_module in original_modules.items():
        setattr(attn_module, layer_name, original_module)
...
def build_quantized_target_modules(
    ...
    logger.info("开始为目标 Q/K/V/O 线性层构建 Ours 量化器。")
...
def build_sq_target_modules(
    ...
    logger.info("开始为目标 Q/K/V/O 线性层构建 SQ-XW 对照组。")
...
def run_last_block_qkvo_experiment(config: ExperimentConfig) -> ExperimentArtifacts:
    ...
    logger.info("说明：本版本沿用 v8 的评测口径；仅将最后一个 transformer block 的 Q/K/V/O 量化，并加入独立 SQ-XW 对照组。")
    ...
    attn_module, target_modules = get_last_block_attention_targets(
        model,
        target_linear_names=config.target.target_linear_names,
        block_index=config.target.block_index,
    )
    logger.info("目标层定位完成 | block_index=%s target_layers=%s", config.target.block_index, list(target_modules.keys()))
    ...
    original_modules = replace_target_modules(attn_module, sq_modules)
    try:
        sq_baseline_ppl, sq_eval_stats = evaluate_perplexity_sliding_window(
            ...
            max_eval_tokens=config.data.eval_num_tokens,
            logger=logger,
            tag="sq_last_block_qkvo",
        )
        ppl_eval_info["sq_last_block_qkvo"] = sq_eval_stats
        logger.info("SQ-XW 对照组评测完成。")
    finally:
        restore_target_modules(attn_module, original_modules)
    ...
    original_modules = replace_target_modules(attn_module, quantized_modules)
    try:
        quantized_ppl, quantized_eval_stats = evaluate_perplexity_sliding_window(
            ...
            max_eval_tokens=config.data.eval_num_tokens,
            logger=logger,
            tag="ours_last_block_qkvo",
        )
        ppl_eval_info["ours_last_block_qkvo"] = quantized_eval_stats
    ...
    with open(output_dir / "last_block_qkvo_results.json", "w", encoding="utf-8") as f:
...
def main() -> None:
    ...
    artifacts = run_last_block_qkvo_experiment(config)
