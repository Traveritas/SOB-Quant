...
# ==========================================
# 量化核心逻辑（考虑 X 与 W 的联合量化）
# ==========================================
def quantize_weight_and_activation(X_tensor, W_tensor, max_iters=50, tol=1e-5, beta=1.0):
    d, N = X_tensor.shape
    ...
    for it in range(max_iters):
        ...
        U = update_U_procrustes(X_tensor, Z_x, Lambda_x, D_x, W_tensor, Z_w, Lambda_w, D_w, beta=beta)
        ...
        if abs(prev_loss - total_loss) < tol:
            print(f"      [提前终止] 迭代 {it+1}/{max_iters}, Loss变化量小于 {tol}")
            break
...
# ==========================================
# 1) 改造 ResNet-18 用于 CIFAR-10
# ==========================================
def create_resnet18_cifar10():
    """
    修改原版ResNet18: 
    CIFAR10图像是3通道 32x32，原版7x7和最大池化会丢失太多信息。
    将 conv1 替换为 3x3 卷积，去掉 maxpool。
    """
    model = models.resnet18(pretrained=False)
    # CIFAR-10 专用开头: 3通道输入，3x3卷积，步长1，不加池化
    model.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
    model.maxpool = nn.Identity()  # 移除MaxPool
    
    # 修改 FC 部分：依旧使用隐层增加大量参数使得量化效果更明显
...
def train_baseline(model, train_loader, device, epochs=15, max_train_batches=None):
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4) # 加了一点权重衰减
    # 加入 CosineAnnealingLR 学习率调度器以实现更好的收敛
    ...
    with torch.no_grad():
        for name, module in model.named_modules():
            ...
            W_quantized, stats = quantize_weight_and_activation(
                X_original, W_original, max_iters=max_iters, beta=10.0
            )
...
def plot_quantization_history(layer_stats, save_dir="plots"):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    for layer_name, stats in layer_stats.items():
        history_total = stats.get("history", [])
        history_x = stats.get("history_x", [])
        history_w = stats.get("history_w", [])
        
        if not history_total:
            continue

        epochs = range(1, len(history_total) + 1)

        plt.figure(figsize=(10, 6))
        plt.plot(epochs, history_total, label="Total Loss", linewidth=2.5, color='black', alpha=0.8)
        plt.plot(epochs, history_x, label="Loss (Activation X)", linestyle='--', color='blue', alpha=0.7)
        plt.plot(epochs, history_w, label="Loss (Weight W)", linestyle='--', color='red', alpha=0.7)

        plt.title(f"Quantization Convergence - Layer {layer_name}\n"
                  f"(d={stats['d']}, m={stats['m']}, N={stats['N']}, Iters={len(history_total)})", fontsize=14)
        ...
        save_path = os.path.join(save_dir, f"convergence_{layer_name}_CIFAR10.png")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("=== 1) 创建并训练 ResNet-18 (改) 用于 CIFAR-10 基线模型 ===")
    print("=" * 70)
    ...
    # 提升 epochs 到 15 并加入 lr_scheduler, 让它更加贴近收敛限制
    train_baseline(baseline_model, train_loader, device, epochs=15)
    ...
    print("\n" + "=" * 70)
    print("=== 3) 应用联合量化 (X+W) 到所有满足限制的 FC 层 ===")
    print("=" * 70)
    ...
    for layer_name, stats in layer_stats.items():
        hist_str = f"迭代次数 {len(stats['history'])}"
        if stats['history']:
            hist_str += f", 最终Total Loss {stats['history'][-1]:.6f}"
        print(f"  {layer_name}: MSE={stats['mse']:.6f}, MAE={stats['mae']:.6f}, d={stats['d']}, m={stats['m']}, {hist_str}")
    ...
    print(f"\n【开始生成量化全过程的图表分析】...")
    plot_quantization_history(layer_stats, save_dir="quantization_plots")
