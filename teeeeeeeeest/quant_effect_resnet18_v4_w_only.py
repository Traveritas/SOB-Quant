...
# ==========================================
# M-Step：更新正交矩阵 U (仅针对 W)
# ==========================================
def update_U_procrustes_w_only(W, Z_w, Lambda_w, D_w):
    Y_w = Lambda_w @ Z_w
    M = W @ D_w @ Y_w.T
...
# 量化核心逻辑（仅考虑 W 的量化）
# ==========================================
def quantize_weight_only(W_tensor, max_iters=50, tol=1e-5):
    """
    仅针对权重 W 的量化 (在 GPU / device 上运算)。
    返回重建的权重矩阵 W_approx
    """
    d, M = W_tensor.shape
    ...
    for it in range(max_iters):
        ...
        # M-Step 仅对 W
        U = update_U_procrustes_w_only(W_tensor, Z_w, Lambda_w, D_w)
    ...
    stats = {
        "mse": mse,
        "mae": mae,
        "history": history_w, # 为了兼容性保持 history
        "history_w": history_w,
...
def apply_quantization_on_linear_layers(model, device, max_iters=50):
    """
    仅针对 W 对所有 Linear 层量化权重并写回模型。
    不需要传入 data_loader 和收集 X 的步骤。
    """
    layer_stats = {}
    ...
    with torch.no_grad():
        for name, module in model.named_modules():
            ...
            # 估算量化参数: U + Lambda + Z (假设码本大小4即2-bit)
            ...
            print(f"  量化层 {name} (d={d}, W={m}, W-Only)...")
            
            W_quantized, stats = quantize_weight_only(
                W_original, max_iters=max_iters
            )

            module.weight.data = W_quantized.T.clone()
            layer_stats[name] = stats
            print(f"  ✓ 完成层 {name} 量化 (W-Only)")
...
def plot_quantization_history(layer_stats, save_dir="plots"):
    ...
    for layer_name, stats in layer_stats.items():
        ...
        epochs = range(1, len(history_w) + 1)

        plt.figure(figsize=(10, 6))
        plt.plot(epochs, history_w, label="Loss (Weight W)", linewidth=2.5, color='red', alpha=0.8)

        plt.title(f"Quantization Convergence (W-Only) - Layer {layer_name}\n"
                  f"(d={stats['d']}, m={stats['m']}, Iters={len(history_w)})", fontsize=14)
        ...
        save_path = os.path.join(save_dir, f"convergence_{layer_name}_W_only.png")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("=== 1) 创建并训练 ResNet-18 基线模型（含大 FC 层）===")
    print("=" * 70)
    ...
    for layer_name, stats in layer_stats.items():
        ...
        if stats['history']:
            hist_str += f", 最终 Loss {stats['history'][-1]:.6f}"
