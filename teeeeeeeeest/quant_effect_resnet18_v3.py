...
def quantize_weight_and_activation(X_tensor, W_tensor, max_iters=50, tol=1e-5, beta=1.0):
    """
    基于输入数据 X 和权重 W 的联合量化 (在 GPU / device 上运算)。
    返回重建的权重矩阵 W_approx (保留了同样的维度)
    """
    d, N = X_tensor.shape
...
# ==========================================
# 1) 改造 ResNet-18 用于 MNIST + 更大的 FC 部分
# ==========================================
def create_resnet18_with_large_fc():
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if batch_idx % 50 == 0:
                ...
                print(f"Epoch {epoch + 1} | step {batch_idx:4d} | seen {seen:5d} | loss {avg_loss:.4f}")
...
def collect_activation_data(model, data_loader, device, num_batches=1):
    ...
    def hook_fn(name):
        def hook(module, input, output):
            ...
            x = input[0].detach() # 保持在当前 device 上
    ...
    for name in activation_data:
        activation_data[name] = torch.cat(activation_data[name], dim=0).T  # 转置为 (in_features, N)
...
def plot_quantization_history(layer_stats, save_dir="plots"):
    ...
    for layer_name, stats in layer_stats.items():
        ...
        save_path = os.path.join(save_dir, f"convergence_{layer_name}.png")
...
def main():
    ...
    print("=" * 70)
    baseline_model = create_resnet18_with_large_fc().to(device)
    ...
    print("\n" + "=" * 70)
    print("=== 3) 应用量化到所有 FC 层（考虑X进行联合优化）===")
    print("=" * 70)
