...
def create_resnet18_with_large_fc():
    """
    ResNet-18，适配 MNIST 单通道输入 + 更大的 FC 隐层。
    原模型 FC: 512->10
    改为: 512->1024->10（增加可量化的大矩阵）
    """
...
def apply_quantization_on_all_layers(model, device, max_iters=3):
    """
    对所有 Conv2d 和 Linear 层量化权重并写回模型。
    - Conv2d: (out_channels, in_channels, H, W) -> reshape -> 量化 -> reshape 回
    - Linear: (out_features, in_features) -> 量化
    返回每层的重建误差统计。
    """
    layer_stats = {}
    ...
    with torch.no_grad():
        for name, module in model.named_modules():
            ...
            if is_conv:
                # Conv2d 权重形状: (out_channels, in_channels, H, W)
                W_original = module.weight.data.cpu().numpy()
                out_c, in_c, kh, kw = W_original.shape
                
                # Reshape to 2D: (out_channels, in_channels*H*W)
                W_2d = W_original.reshape(out_c, -1)
                d, m = W_2d.shape
                ...
                W_original = module.weight.data.cpu().numpy()
                W_2d = W_original.T  # 转置为 (in_features, out_features)
                d, m = W_2d.shape
                original_shape = W_original.shape
            ...
            print(f"  量化层 {name} {original_shape}...", end=" ", flush=True)
            W_quantized, stats = quantize_weight_matrix(W_2d, max_iters=max_iters)

            # 写回
            if is_conv:
                W_reconstructed = W_quantized.reshape(original_shape)
                W_tensor = torch.from_numpy(W_reconstructed).to(dtype=module.weight.data.dtype, device=device)
                module.weight.data = W_tensor
            else:  # Linear
                W_reconstructed = W_quantized.T  # 转回原形状
                W_tensor = torch.from_numpy(W_reconstructed).to(dtype=module.weight.data.dtype, device=device)
                module.weight.data = W_tensor
...
def estimate_quantized_storage_bits(model, layer_stats, float_bits=32):
    """
    估算量化表示的存储位数：
    - 对每个量化层: U(d*d float) + Lambda(d float) + Z索引(d*m*2bit)
    - 码本: 共享，全局一次 codebook_size * float_bits
...
def main():
    ...
    print("\n" + "=" * 70)
    print("=== 3) 应用量化到所有卷积层和 FC 层 ===")
    print("=" * 70)
    quant_model = copy.deepcopy(baseline_model)
    layer_stats = apply_quantization_on_all_layers(quant_model, device, max_iters=3)

    print(f"\n量化了 {len(layer_stats)} 层")
    total_quantized_params = 0
    for layer_name, stats in layer_stats.items():
        param_count = stats['d'] * stats['m']
        total_quantized_params += param_count
        print(
            f"  {layer_name}: d={stats['d']}, m={stats['m']}, "
            f"params={param_count:,}, MSE={stats['mse']:.6f}"
        )
    
    quant_ratio = 100.0 * total_quantized_params / total_params
    print(f"量化参数占比: {quant_ratio:.2f}%")
    ...
    print("说明")
    print("=" * 70)
    print(f"• 模型: ResNet-18，{total_params/1e6:.2f}M 参数")
    print(f"• 量化策略: 所有卷积层 + FC 层（共 {len(layer_stats)} 层，占 {quant_ratio:.2f}% 参数）")
    print("• 分解表示: U(d²) + Lambda(d) + Z 索引(2-bit) - 共享码本")
    print("• 卷积层处理: 4D -> 2D reshape -> 量化 -> reshape 回 4D")
    print("• 测速: warmup=5, 20个 batch，重复 20 次，输出均值±标准差")
    print("• GPU: RTX4060，已启用 CUDA 加速")
