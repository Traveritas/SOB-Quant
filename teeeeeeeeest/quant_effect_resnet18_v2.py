...
# ==========================================
def quantize_weight_matrix(W_numpy, max_iters=50, tol=1e-5):
    """
    ...
    for it in range(max_iters):
        ...
        if abs(prev_mse - mse) < tol:
            print(f"      [提前终止] 迭代 {it+1}/{max_iters}, 当前 MSE: {mse:.6f}, 变化量小于 tol ({tol})")
            break
        prev_mse = mse
...
def apply_quantization_on_linear_layers(model, device, max_iters=50):
    ...
    with torch.no_grad():
        for name, module in model.named_modules():
            ...
            print(f"  量化层 {name} ({d}x{m})...")
            W_quantized, stats = quantize_weight_matrix(W_original, max_iters=max_iters)

            # 写回时再转置回来，并确保在正确的设备和dtype上
            W_tensor = torch.from_numpy(W_quantized.T).to(dtype=module.weight.data.dtype, device=device)
...
def main():
    ...
    for name, module in baseline_model.named_modules():
        if isinstance(module, nn.Linear):
            print(f"  FC 层: {name} -> in={module.in_features}, out={module.out_features}")
    ...
    layer_stats = apply_quantization_on_linear_layers(quant_model, device, max_iters=50)

    print(f"\n量化了 {len(layer_stats)} 层:")
    for layer_name, stats in layer_stats.items():
        hist_str = f"迭代次数 {len(stats['history'])}"
        if stats['history']:
            hist_str += f", 最终MSE {stats['history'][-1]:.6f}"
        print(
            f"  {layer_name}: MSE={stats['mse']:.6f}, MAE={stats['mae']:.6f}, "
            f"d={stats['d']}, m={stats['m']}, {hist_str}"
        )
    ...
    print("说明")
    print("=" * 70)
    print(f"• 模型: ResNet-18 + 大 FC 隐层 (512x1024, 1024x10)，{total_params/1e6:.2f}M 参数")
