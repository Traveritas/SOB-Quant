...
class ArtifactWriter:
    ...
    def write_run(self, artifacts: RunArtifacts) -> None:
        if SAVE_PER_RUN_JSON:
            _atomic_write_json(self.output_dir / RUN_JSON_NAME, asdict(artifacts))
        ...
        _atomic_write_text(self.output_dir / RUN_SUMMARY_NAME, summary_base + "\n" + "\n".join(extra_lines))
    ...
    def write_sweep_summary(self, rows: List[SweepSummaryRow]) -> None:
        ...
        for row in rows:
            lines.append(
                ...
                f"ours={row.quantized_ppl:.6f} | sq={row.sq_baseline_ppl:.6f} | "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="QKVO refactored sweep runner with sbatch-array support")
    ...
    parser.add_argument("--rebuild-summary", action="store_true", help="不跑实验，只扫描已有 res.json 重建 sweep 汇总")
