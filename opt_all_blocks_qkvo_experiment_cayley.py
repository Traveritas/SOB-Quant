...
class QuantizerConfig:
    ...
    u_update_method: str = "cayley"
    cayley_step_size: float = 0.1
    cayley_max_inner_iters: int = 10
    cayley_tol: float = 1e-6
    cayley_step_decay: float = 0.5
    cayley_min_step_size: float = 1e-5
    cayley_backtracking: bool = True
...
def build_analysis_summary(artifacts: "ExperimentArtifacts") -> str:
    lines: List[str] = []
    lines.append("实验结果分析")
    quant_cfg = artifacts.config.get("quant", {})
    latent_mode = quant_cfg.get("latent_mode", "discrete")
    lines.append(f"- latent_mode: {latent_mode}")
    lines.append(f"- U update: {quant_cfg.get('u_update_method', 'procrustes')}")
    if quant_cfg.get("u_update_method", "procrustes") == "cayley":
        lines.append(
            "- Cayley超参数: "
            f"step_size={quant_cfg.get('cayley_step_size')} "
            f"inner_iters={quant_cfg.get('cayley_max_inner_iters')} "
            f"tol={quant_cfg.get('cayley_tol')}"
        )
...
class LatticeLinearQuantizer:
    ...
    def _procrustes_objective_matrix(
        self,
        lambda_x: torch.Tensor,
        lambda_w: torch.Tensor,
        SXZx: torch.Tensor,
        ...
        return SXZx @ torch.diag(lambda_x) + self.config.beta * SWZw @ torch.diag(lambda_w)

    def _cayley_gradient(
        self,
        U: torch.Tensor,
        lambda_x: torch.Tensor,
        lambda_w: torch.Tensor,
        ...
        lambda_x_diag = torch.diag(lambda_x)
        lambda_w_diag = torch.diag(lambda_w)
        term_x = lambda_x_diag @ SZZx @ lambda_x_diag
        term_w = lambda_w_diag @ SZZw @ lambda_w_diag
        cross = self._procrustes_objective_matrix(lambda_x, lambda_w, SXZx, SWZw)
        return 2.0 * U @ (term_x + self.config.beta * term_w) - 2.0 * cross

    def _cayley_step(self, U: torch.Tensor, omega: torch.Tensor, step_size: float) -> torch.Tensor:
        d = U.shape[0]
        eye = torch.eye(d, dtype=U.dtype, device=U.device)
        left = eye + 0.5 * step_size * omega
        right = (eye - 0.5 * step_size * omega) @ U
        return torch.linalg.solve(left, right)

    def _cayley_score(self, U: torch.Tensor, cross: torch.Tensor) -> torch.Tensor:
        return torch.trace(U.T @ cross)
    ...
    def _update_U(
        ...
        method = getattr(self.config, "u_update_method", "cayley").lower()
        cross = self._procrustes_objective_matrix(lambda_x, lambda_w, SXZx, SWZw)

        if method == "procrustes":
            P, _, Qt = torch.linalg.svd(cross, full_matrices=False)
            return P @ Qt
        if method != "cayley":
            raise ValueError(f"Unsupported u_update_method: {self.config.u_update_method}")

        score = self._cayley_score(U, cross)
        best_score = score
        best_U = U
        base_step = float(self.config.cayley_step_size)

        for inner_idx in range(1, self.config.cayley_max_inner_iters + 1):
            grad_U = self._cayley_gradient(U, lambda_x, lambda_w, SXZx, SWZw, SZZx, SZZw)
            omega = grad_U @ U.T - U @ grad_U.T
            omega = 0.5 * (omega - omega.T)

            grad_norm = float(torch.linalg.norm(grad_U).item())
            skew_norm = float(torch.linalg.norm(omega).item())
            if skew_norm < self.config.cayley_tol:
                if self.logger is not None:
                    self.logger.info(
                        "tag=%s outer_iter=%s cayley inner=%02d 提前停止 | skew_norm=%.6e",
                        tag,
                        str(outer_iter),
                        inner_idx,
                        skew_norm,
            ...
            step_size = base_step
            accepted = False
            min_step = float(self.config.cayley_min_step_size)
            step_decay = float(self.config.cayley_step_decay)

            while step_size >= min_step:
                candidate = self._cayley_step(U, omega, step_size)
                candidate_score = self._cayley_score(candidate, cross)
                if (not self.config.cayley_backtracking) or candidate_score >= score - self.config.cayley_tol:
                    U = candidate
                    ...
                    if candidate_score > best_score:
                        ...
                        best_U = candidate
                    if self.logger is not None and (inner_idx == 1 or inner_idx == self.config.cayley_max_inner_iters):
                        self.logger.info(
                            "tag=%s outer_iter=%s cayley inner=%02d | step=%.4e grad=%.6e skew=%.6e score=%.6e",
                            tag,
                            str(outer_iter),
                            inner_idx,
                            step_size,
            ...
            if not accepted:
                if self.logger is not None:
                    self.logger.info(
                        "tag=%s outer_iter=%s cayley inner=%02d 未接受更新，返回当前最优 U | grad=%.6e skew=%.6e",
                        tag,
                        str(outer_iter),
                        inner_idx,
                        grad_norm,
            ...
            if abs(float(score.item()) - float(best_score.item())) < self.config.cayley_tol and skew_norm < 10.0 * self.config.cayley_tol:
                break
    ...
    def fit(self, X: torch.Tensor, W: torch.Tensor, tag: str = "") -> QuantizationState:
        ...
        for t in range(1, self.config.max_iters + 1):
            ...
            if self.logger is not None and (t == 1 or t % self.config.log_every == 0):
                self.logger.info(
                    "tag=%s iter=%03d | J=%.6f J_x=%.6f J_w=%.6f rel_change=%.6e | "
        ...
        return QuantizationState(
            ...
            u_update_method=getattr(self.config, "u_update_method", "cayley"),
        )
...
class QuantizedLinear(nn.Module):
    def __init__(self, state: QuantizationState, bias: Optional[torch.Tensor] = None):
        ...
        self.u_update_method = state.u_update_method
        if bias is None:
            self.bias = None
        else:
...
def run_all_blocks_qkvo_experiment(config: ExperimentConfig) -> ExperimentArtifacts:
    ...
    logger.info("说明：本版本将 U-subproblem 的闭式 Procrustes 更新替换为 Cayley 迭代；将所有目标 transformer blocks 的 Q/K/V/O 量化，并加入独立 SQ-XW 对照组。")
    logger.info(
        "当前 latent_mode=%s | u_update_method=%s | cayley_step_size=%.4f | cayley_inner_iters=%d",
        config.quant.latent_mode,
        config.quant.u_update_method,
        config.quant.cayley_step_size,
        config.quant.cayley_max_inner_iters,
    )
    ...
    target_info = {
        ...
        "u_update_method": config.quant.u_update_method,
        "cayley_step_size": config.quant.cayley_step_size,
        "cayley_max_inner_iters": config.quant.cayley_max_inner_iters,
    }
...
def parse_int_tuple_arg(raw: Optional[str]) -> Optional[Tuple[int, ...]]:
    if raw is None:
        return None
    value = raw.strip()
...
def parse_str_tuple_arg(raw: Optional[str]) -> Optional[Tuple[str, ...]]:
    if raw is None:
        return None
    value = raw.strip()
    if not value:
...
def build_config_from_cli() -> ExperimentConfig:
    parser = argparse.ArgumentParser(
        description="OPT all-blocks QKVO experiment with Cayley U-update.",
    )
    parser.add_argument("--block_indices", type=str, default=None, help='Comma-separated block indices, e.g. "10,11". Use "all" for all blocks.')
    parser.add_argument("--target_linear_names", type=str, default=None, help='Comma-separated target linear names, e.g. "k_proj,v_proj".')
    parser.add_argument("--run_mode", type=str, default=None, choices=["discrete", "continuous", "both"])
    parser.add_argument("--output_dir", type=str, default=None)
    parser.add_argument("--beta", type=float, default=None)
    parser.add_argument("--init_mode", type=str, default=None, choices=["pca", "random"])
    parser.add_argument("--error_mode", type=str, default=None, choices=["relative", "absolute"])
    parser.add_argument("--latent_mode", type=str, default=None, choices=["discrete", "continuous"])
    parser.add_argument("--u_update_method", type=str, default=None, choices=["cayley", "procrustes"])
    parser.add_argument("--cayley_step_size", type=float, default=None)
    parser.add_argument("--cayley_max_inner_iters", type=int, default=None)
    parser.add_argument("--cayley_tol", type=float, default=None)
    parser.add_argument("--seed", type=int, default=None)
    args = parser.parse_args()

    config = ExperimentConfig()

    block_indices = parse_int_tuple_arg(args.block_indices)
    if args.block_indices is not None:
        config.target.block_indices = block_indices

    target_linear_names = parse_str_tuple_arg(args.target_linear_names)
    if target_linear_names is not None:
        config.target.target_linear_names = target_linear_names
    ...
    if args.u_update_method is not None:
        config.quant.u_update_method = args.u_update_method
    if args.cayley_step_size is not None:
        config.quant.cayley_step_size = args.cayley_step_size
    if args.cayley_max_inner_iters is not None:
        config.quant.cayley_max_inner_iters = args.cayley_max_inner_iters
    if args.cayley_tol is not None:
        config.quant.cayley_tol = args.cayley_tol
